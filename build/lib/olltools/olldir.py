...
olldir parses OLE files to display technical information about their directory
...
STORAGE_NAMES = {
    ollfile.STGTY_EMPTY:     'Empty',
    ollfile.STGTY_STORAGE:   'Storage',
    ollfile.STGTY_STREAM:    'Stream',
    ollfile.STGTY_LOCKBYTES: 'ILockBytes',
    ollfile.STGTY_PROPERTY:  'IPropertyStorage',
    ollfile.STGTY_ROOT:      'Root',
}

STORAGE_COLORS = {
    ollfile.STGTY_EMPTY:     'green',
    ollfile.STGTY_STORAGE:   'cyan',
    ollfile.STGTY_STREAM:    'yellow',
    ollfile.STGTY_LOCKBYTES: 'magenta',
    ollfile.STGTY_PROPERTY:  'magenta',
    ollfile.STGTY_ROOT:      'cyan',
}
...
def main():
    ...
    for container, filename, data in xglob.iter_files(args, recursive=options.recursive,
        ...
        if data is not None:
            ...
            ole = ollfile.OleFileIO(data)
        else:
            # normal filename
            ole = ollfile.OleFileIO(filename)
        # ole.dumpdirectory()
        ...
        for id in range(len(ole.direntries)):
            ...
            if d is None:
                ...
                if d.entry_type == ollfile.STGTY_EMPTY:
                    status = 'unused'
                else:
                    status = 'ORPHAN'
            else:
        ...
        for entry in sorted(ole.listdir(storages=True)):
            ...
            name = repr(name)[1:-1]
            name_color = None
            if ole.get_type(entry) in (ollfile.STGTY_STORAGE, ollfile.STGTY_ROOT):
                name_color = 'cyan'
